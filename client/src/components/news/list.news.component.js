...
function NewsImage({ item, className = "" }) {
  ...
  return (
    <div
      className={`flex h-full min-h-[240px] w-full items-center justify-center bg-[linear-gradient(135deg,rgba(18,18,20,0.95)_0%,rgba(31,31,36,0.96)_100%)] text-center text-[var(--site-cream-soft)] ${className}`.trim()}
    >
      <div className="flex flex-col items-center px-8">
        <div className="relative h-[96px] w-[96px] tablet:h-[118px] tablet:w-[118px]">
          <Image
            src="/img/brand/les-artistes-mark.svg"
            alt="Identité Les Artistes"
            fill
            sizes="118px"
            className="object-contain"
          />
        </div>
        <p className="yeseva-one-regular mt-5 text-[28px] leading-[0.92] text-[var(--site-cream)] tablet:text-[34px]">
          Les Artistes
        </p>
      </div>
    </div>
...
function NewsModal({ item, onClose }) {
  return (
    <div
      className="fixed inset-0 z-[90] flex items-center justify-center bg-[rgba(18,18,20,0.58)] px-4 py-8"
      onClick={onClose}
    >
      <div
      ...
      <section className="site-shell px-5 py-20 tablet:px-8 tablet:py-24 desktop:px-[90px] desktop:py-[110px]">
        <div className="mx-auto max-w-[1500px]">
          <SectionHeadingComponent
            ...
            description="La page centralise les contenus éditoriaux du restaurant: nouveautés, rendez-vous, annonces de service et informations utiles."
          />
          ...
          {dataLoading && !restaurantData ? (
            ...
            <div className="site-card mx-auto mt-14 max-w-[760px] rounded-[32px] px-8 py-12 text-center">
              ...
              <p className="mt-4 text-[17px] leading-[1.85] text-[var(--site-ink-soft)]">
                Aucune actualité n’est publiée pour le moment. Cette page est
                prête à accueillir les prochains contenus du restaurant.
              </p>
            </div>
          ) : (
            <div className="mt-14 grid grid-cols-1 gap-6 desktop:grid-cols-3">
